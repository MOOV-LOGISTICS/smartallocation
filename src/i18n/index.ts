...
export const I18N = {
  en: {
    ...
    step: {
      ...
      titles: {
        ...
        2: 'Match Carrier (Initial)',
        3: 'Match Available Carrier',
        4: 'Match Vessel Voyage',
        5: 'Auto Pre-Assign'
      },
      rules: {
        1: 'Buffer = FOB_WEEK − CRD_WEEK. Allowed range: 0–4 weeks → PASS. Buffer > 4 weeks → check Early Shipment List (PASS if approved; ON_HOLD if not found). Buffer < 0 (CRD after FOB) → EXCEPTION.',
        2: 'Query BOOKING_MATRIX[POL → POD] → carrier list; EXCEPTION if result is empty (no carrier registered for this lane).',
        3: 'Estimate ETD window = CRD + 12~20 days → derive Allocation Week. For each matched carrier, check ALLOCATION_DATA[carrier][ALLOCATION_WEEK][region] ≥ required TEU; retain carriers with sufficient quota; EXCEPTION if none qualify.',
        ...
        5: 'Write record to t_pre_assign (poId, carrier, vessel, voyage, etd, eta, fnd); update PO status → ASSIGNED; notify Customer Service team.'
      },
      reasons: {
        s1Pass: 'Booking matrix matched {count} carrier(s) for {lane}. Proceeding to allocation check.',
        s1Fail: 'No carrier registered in BOOKING_MATRIX for POL {pol}. This origin is not in any active lane.',
        s2Pass: 'Allocation week {week} (CRD+12~20d, {region}): carriers with sufficient quota confirmed.',
        s2Fail: 'Allocation week {week} (CRD+12~20d): all carriers exhausted — none can accept {teu} TEU.',
        s3Pass: 'CRD/FOB buffer is {buffer} weeks (within 0–4 week window). PO is eligible to proceed.',
        s3PassEarlyShipment: 'CRD/FOB buffer is {buffer} weeks (>4 weeks). LOT found in Early Shipment List and approved — PO is eligible.',
        s3OnHold: 'CRD ({crdWeek}) and FOB ({fobWeek}) are only {buffer} week(s) apart — below the 4-week minimum. PO placed ON_HOLD.',
        s3OnHoldTooEarly: 'CRD ({crdWeek}) is {buffer} weeks before FOB ({fobWeek}) — exceeds the 4-week maximum. LOT not found in Early Shipment List. PO placed ON_HOLD.',
        s3ExceptionCrdLater: 'CRD week ({crdWeek}) is later than FOB week ({fobWeek}) — cargo will not be ready before vessel departure. EXCEPTION: needs human to resolve.',
        ...
        s4FailTie: 'Rank-1 and Rank-2 voyages share identical ETD and ETA — cannot automatically select unique best. Manual review required.',
        s5Pass: 'Pre-assignment completed. Record written to t_pre_assign and Customer Service notified.',
    ...
    exceptionReasons: {
      ...
      noVoyage: 'No vessel voyage found within the CRD+12d to CRD+35d ETD window with ETA ≤ LDD.',
  ...
  zh: {
    ...
    page: { title: '预排船', subtitle: 'PO 导入与自动预排船', heroTitle: 'PO 排船看板，', heroTitleAccent: '已自动化。', heroBody: '从 BI 实时拉取 PO 列表，按 6 步逻辑自动匹配最优船公司、航线与航次。点击任意行可查看完整 trace log；选中后批量触发预排船。' },
    ...
    step: {
      ...
      rules: {
        1: 'Buffer = FOB_WEEK − CRD_WEEK。允许范围：0–4 周 → PASS。Buffer > 4 周 → 查 Early Shipment List（在列则通过，不在则 ON_HOLD）。Buffer < 0（CRD 晚于 FOB）→ EXCEPTION。',
        2: '查询 BOOKING_MATRIX[POL → POD]，获取可用船公司列表；若结果为空则 EXCEPTION（该航线无承运商覆盖）。',
        3: '预估 ETD 窗口 = CRD + 12~20 天 → 推算 Allocation Week。对每家候选船公司，检查 ALLOCATION_DATA[carrier][ALLOCATION_WEEK][region] ≥ 需求 TEU；保留有足够配额的船公司；若无则 EXCEPTION。',
        4: '查询 VESSEL_SCHEDULES，筛选 ETD ∈ [CRD+12d, CRD+20d] 且 ETA ≤ LDD 的航次；通过 FND_RULES 解析 FND；若无符合航次则 EXCEPTION。再按 priority 升序 → ETA 升序 → ETD 降序排列；若第 1 名与第 2 名并列则 EXCEPTION。',
        5: '写入 t_pre_assign 记录（poId、carrier、vessel、voyage、etd、eta、fnd）；更新 PO 状态为 ASSIGNED；通知 Customer Service 团队。'
      },
      reasons: {
        s1Pass: '航线矩阵为 {lane} 匹配到 {count} 家船公司，继续执行配额检查。',
        s1Fail: 'BOOKING_MATRIX 中未注册 POL {pol} 的任何船公司，该起运港不在任何活跃航线中。',
        s2Pass: '配额周 {week}（{region}）：确认有足够配额的船公司。',
        s2Fail: '配额周 {week} 所有船公司均已耗尽：无法接受 {teu} TEU。',
        s3Pass: 'CRD/FOB 间隔为 {buffer} 周（在 0–4 周范围内），PO 符合条件。',
        s3PassEarlyShipment: 'CRD/FOB 间隔为 {buffer} 周（>4 周）。LOT 已在 Early Shipment List 批准，PO 符合条件。',
        s3OnHold: 'CRD（{crdWeek}）与 FOB（{fobWeek}）仅相差 {buffer} 周，低于 4 周最低要求。PO 转为 ON_HOLD。',
        s3OnHoldTooEarly: 'CRD（{crdWeek}）比 FOB（{fobWeek}）早 {buffer} 周，超出 4 周上限。LOT 未在 Early Shipment List 中。PO 转为 ON_HOLD：请求过早。',
        s3ExceptionCrdLater: 'CRD 周（{crdWeek}）晚于 FOB 周（{fobWeek}）——货物备货日期在计划 FOB 出发后。EXCEPTION：需人工处理。',
        s4Pass: '在 ETD 窗口内找到并排序航次，已选出唯一最优。FND 已成功解析。',
        s4FailNoVoyage: '在 {lane} 的 ETD 窗口 [{start} → {end}] 内且 ETA ≤ LDD 的航次未找到。',
        s4FailTie: '第 1 名和第 2 名航次的 ETD 和 ETA 完全相同——无法自动选出唯一最优，需人工介入。',
        s5Pass: '预排船完成。记录已写入 t_pre_assign，Customer Service 团队已收到通知。',
