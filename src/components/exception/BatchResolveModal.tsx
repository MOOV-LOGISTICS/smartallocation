...
interface BatchResolveModalProps {
  open: boolean;
  count: number;
  onClose: () => void;
...
export function BatchResolveModal({ open, count, onClose, onSubmit, lang }: BatchResolveModalProps) {
          ...
          <div className="fixed inset-0 flex items-center justify-center z-[61] pointer-events-none">
            ...
            >
              <div className="px-6 py-4 border-b border-[#DEE5EC] flex items-center justify-between">
                <h2 className="text-lg font-semibold text-[#0F2A3F]">{t(lang, 'batchResolve.title')}</h2>
                <button onClick={handleClose} className="p-1 hover:bg-[#F8FAFC] rounded text-[#4A5A6E] transition-colors">
                  <IconClose />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-5">
                <div className="bg-[#F8FAFC] rounded-lg p-4 border border-[#DEE5EC]">
                  <div className="text-sm font-medium text-[#0F2A3F]">
                    {t(lang, 'batchResolve.summary', { n: count })}
                  </div>
                </div>
